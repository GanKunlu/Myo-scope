...
class AnimationHandler(Handler):
	def init(self, info):
		...
		info.object.timer = Timer(120,info.object.on_timer)
...
class MYOGrapher(HasTraits):
	sEMG_length = Range(100,500,100)
	Angle_length = Range(80,300,100)
	sEMG_names = List(Str)
	...
	stream_emg = List(Array)
	stream_Angle = List(Array)
	EMG_StreamWin = Array
	Angle_StreamWin = Array
	window_size = Range(5,25,8)
	emg_posit = Int
	angle_posit = Int
	traits_view=View(	
			HSplit(# HSplit分为左右两个区域，中间有可调节宽度比例的调节手柄
				...
				VSplit(
					...
					Item("window_size", label=u"活动窗尺寸")
					),
				VGroup(
	...
	def init_angledata(self):
		for in_myo in range(2):
			self.stream_Angle[in_myo] = np.zeros(self.Angle_length)
	...
	def on_timer(self):
		...
		if len(self.select_emg_names)!= 0:
			if self.emg_posit == self.sEMG_length - self.window_size:
				self.emg_posit = 0
			for name in self.select_emg_names:
				index = self.sEMG_names.index(name)
				self.EMG_StreamWin[0] = myo_device[__myos].emg[index]
				self.stream_emg[index][self.emg_posit : self.emg_posit + self.window_size] = self.EMG_StreamWin
				self.data_EMG[name] = self.stream_emg[index]
			self.emg_posit += 1
