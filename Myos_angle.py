...
edited on 2016.5.31
@Author: Gan
This module have the main function to get 2 Myos' angle in real time.
...
def GetDegreeFromACC(ACCs):
	# get the angle from ACC
	# input type: List(Array),each array in the input list should be the ACC data get from Myo
	# return type: list, return the caculated angle using each myo's ACC data you input in ACCs
	R = 0
	angle=[]
	ACC_angle = []
	for ACC in ACCs:
...
def angle_init(myo,angle,which_myo):
	# get the initial angle from ACC data
	index =0
...
def updataAngle(myo,angle,which_myo,T):
	# caculate the angle use gyroscope's data.
	index = 0
